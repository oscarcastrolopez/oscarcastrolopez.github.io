...
```
# Descripción
Este es un análisis exploratorio de datos con fines didácticos. El análisis es realizado con datos relacionados con Covid-19 reportados por diversos países. Los datos son obtenidos del sitio: <https://ourworldindata.org/coronavirus-source-data>. El conjunto de datos del sitio es actualizado constantemente.

#### Lectura de datos ##
Leemos los datos directamente del sitio para obtener los datos actualizados al día de ejecución del código.
```{r}
...
Filtramos nuestro dataframe original, sólo nos quedamos con las observaciones en donde la columna date sea igual a la fecha máxima. De esta manera tendremos los totales o acumulados actualizados de cada país. Adicionalmente, `r kableExtra::text_spec("eliminamos los países con menos de 1 millón de habitantes", color = "red")`.
...
Para obtener el top 20 ordenamos los datos con la columna total_cases. Guardamos sólo los 20 países con más casos y los 20 países con menos casos. Filtramos sólo las columnas que nos interesan, y mostramos los datos en una tabla y en una gráfica de barras horizontales.
...
Para obtener el top 20 ordenamos los datos con la columna total_cases_per_million. Guardamos sólo los 20 países con más casos y los 20 países con menos casos. Filtramos sólo las columnas que nos interesan y mostramos los datos en una tabla y en una gráfica de Barras horizontales. Cómo México no está en el top 20, lo agregamos al final con la posición correspondiente del ranking.
<div class = "row">
```{r results='hold'}
...
```
Podemos observar que las líneas empiezan a subir a partir de marzo. Modificamos la gráfica con diversas fechas de inicio:
```{r fig.height=8, fig.width=15, warning=FALSE}
...
plot.trend.total_cases(enddate-14, enddate, total_cases.others, paste("Curvas de contagios acumulados de COVID-19 reportados las últimas dos semanas de:",countrieslabels))
```
...
El acumulado de muertes empieza a subir a partir de marzo. Modificamos la gráfica con diversas fechas de inicio:
```{r fig.height=8, fig.width=15, warning=FALSE}
...
plot.trend.total_deaths(enddate-14, enddate, total_deaths.others, "Curvas de muertes acumuladas por COVID-19 reportadas de las últimas dos semanas")
```

Las curvas de Francia, Italia y Reino Unido están planas, se ha estancado la cantidad de muertes atribuidas a COVID-19. Una posible razón es que la pandemia inició primero en Europa y la cantidad de muertes reportadas recientemente es nula o muy baja. Las curvas de México e India siguen en aumento. Quitamos los países europeos y agregamos otros países en donde al parecer la cantidad de muertes atribuidas a COVID-19 aún sigue en aument. Generamos la gráfica con las curvas de muertes acumuladas de las últimas 2 semanas sin contar a Estados Unidos y Brasil:
```{r fig.height=8, fig.width=15, warning=FALSE}
...
#### Contagios y muertes diarías en México
```{r fig.height=8, fig.width=15, warning=FALSE}
...
#### Contagios y muertes totales en México
```{r fig.height=8, fig.width=15, warning=FALSE}
